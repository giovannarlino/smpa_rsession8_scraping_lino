---
title: "R - Scraping Tables - MD Elections"
author: "Data Journalism Handbook - via Derek Willis"
execute:
  echo: true
format:
  html:
...
results <- ag_url %>%
  read_html() %>%
  html_table() #looks for tables in the website 
...
results <- results[[1]] #tell the table you want, sometimes a website might have multiple tables 
...
# Standardize column headers, remove last row for clarity 
# These are splicing formulas 
...
statewide <- results %>% 
  summarise( 
    ...
    total_peroutka = sum(peroutka), 
    total_writeins = sum(write_ins)
  )
...
statewide %>%  
  mutate( 
    total_all = total_peroutka + total_brown + total_writeins,
    pct_peroutka = round_half_up (total_peroutka / total_all * 100, 1), 
    pct_brown = round_half_up (total_brown / total_all * 100, 1), 
    pct_writeins = round_half_up (total_writeins / total_all * 100, 1), 
  )
```
```{r}

results <- results %>%  #have to assign a name to the thing, it is equivalent to saving it, otherwise "R" will not recognize the column 
  mutate( 
    
    total_all = peroutka + brown + write_ins, #now we are doing per number of counties 
    
    pct_peroutka = round_half_up (peroutka / total_all * 100, 1), 
    
    pct_brown = round_half_up (brown/ total_all * 100, 1), 
    
    pct_writeins = round_half_up (write_ins / total_all * 100, 1), 
  )
```
...
results <- results %>%  
  mutate( 
    marginofvic_peroutka = pct_peroutka - pct_brown, 
    winner = if_else(pct_peroutka > pct_brown, "P", "B")
  )
```
